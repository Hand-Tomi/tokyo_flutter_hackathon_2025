...
<<<<<<< HEAD
=======
  /// 스케치 파일 저장 완료 (AirScribble에서 호출) - 전역 상태 업데이트
  void onSketchSaved(String filePath) {
    final latestScene = ref.read(sceneListProvider.notifier).latest;
    if (latestScene != null) {
      ref.read(sceneListProvider.notifier).updateSketch(latestScene.id, filePath);
    }
  }

  /// 장면 확정 - 전역 상태에 Scene 추가
  void onConfirmScenePressed() {
    ...
    ref.read(sceneListProvider.notifier).addScene(
      SceneData(
        id: sceneId,
        storyScript: state.uiState.sttText,
      ),
    );
...
>>>>>>> aed9f24 (feat: 동영상을 슬라이드쇼 방식으로 구현)
}
